...
from invisible_cities.core.fit_functions import SensorSpeResponse
...
def main():
    ...
    bins = np.array(sipmIn.root.HIST.sipmspe_bins)
    ## LED correlated and anticorrelated spectra:
    specsL = np.array(sipmIn.root.HIST.sipmspe).sum(axis=0)
    specsD = np.array(sipmIn.root.HIST.sipmdar).sum(axis=0)
    ...
    b0 = [(0., 0., 0., 0.001),(1e99, 100., 100., 10000.)]
    ## Names for output
    n0 = ['gain', 'poisson', 'sigma', 'chi2', 'saved gain', 'pull_gain', 'sigPe']
    ...
    for ich, (led, dar, gn) in enumerate(zip(specsL, specsD, saved_g)):
        ...
        if len(pDL) <= 1 or abs(pDL[1]-pDL[0]) > 25:
            ## default values
            muSeed = led[(bins>10) & (bins<30)].max() / led[bins<10].max()
            ...
            if z0.size == 0:
                ...
                muSeed = -np.log((led[bins==0]/dar[bins==0])[0])
            else:
            ...
            sigSeed = np.abs(p1[2])
            sd0 = (led.sum(), p1[1], muSeed, sigSeed)
        ##
        ## Fit function
        ...
        if chi > 10:
            ...
            plt.show()
        print('nGau = ', respF.nGau)
        outData.append([chDic['gain'], chDic['poisson'], chDic['sigma'], chi, gn, (gn-chDic['gain'][0])/chDic['gain'][1], chDic['sigma'][0]/chDic['gain'][0]])
